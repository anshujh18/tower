...
function game() {
    ...
    var bullets    = [],
        ...
        explosions = [],
        destroyed  = 0,
        coins      = 0, // Declare coins variable to store score
        record     = 0,
        count      = 0,
        playing    = false,
        gameOver   = false,
            ...
            if(gameOver) {
                ...
                if (dist < 27) {
                    if(e.type == 'click') {
                        ...
                        explosions = [];
                        destroyed  = 0;
                        coins      = 0; // Reset coins when restarting
                        player.deg = 0;
                ...
                for(var j = 0; j < asteroids.length; j++) {
                    if(!asteroids[j].destroyed) {
                        ...
                        if (distance < (((asteroids[j].width/asteroids[j].size) / 2) - 4) + ((19 / 2) - 4)) {
                            destroyed += 1;
                            coins++; // Increment coins when an asteroid is destroyed
    ...
    function start() {
        if(!gameOver) {
            ...
            if(playing) {
                ...
                ctx.textAlign = "left";
                ctx.fillText('Coins: '+coins+'', 20, 30); // Display coins instead of destroyed
